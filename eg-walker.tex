...
\begin{itemize}
    \item Raw editing traces, distributed in our own binary format and as JSON.
    \item Source code for the following tools:
        \begin{itemize}
            \item Our optimised Eg-walker implementation, programmed in Rust.
            \item Our reference CRDT implementation.
            \item Our reference OT (Operational Transform) implementation.
            \item Tools to convert our editing traces to JSON, and to the Yjs and Automerge file formats.
            \item Benchmarking tools to reproduce all experimental data, including:
                \begin{itemize}
                    \item Remote merge timings
                    \item File sizes
                    \item Memory usage
                \end{itemize}
        \end{itemize}
\item The tools we used to automatically generate all the figures in this paper.
\end{itemize}
...
\begin{itemize}
    ...
    \item Plenty of disk space. The rust compiler produces a lot of temporary files - 44GB of them on our system.
\end{itemize}
...
\begin{itemize}
\item File size measurements (Eg-walker vs Yjs and Automerge).
\item Memory usage during merging. We have benchmarks comparing Eg-walker, Yjs, Automerge, our own reference CRDT implementation and a reference Operational Transform implementation. These benchmarks take about 1.5 hours to run.
\item Time taken to merge our editing traces. Again, comparing Eg-walker, Yjs, Automerge, reference CRDT and reference OT implementation on all 7 of our editing traces. These benchmarks take about 12 hours to run.
\end{itemize}
...
These files contain lists of editing transactions. Each transaction contains a non-empty set of \emph{patches} (edits to the document) made by some user agent at some point in time. (Ie, after some other set of transactions have been merged). The file format is described in the \href{https://github.com/josephg/editing-traces/tree/master/concurrent_traces}{\texttt{josephg/editing-traces} repository on Github}).

If you want to benchmark a CRDT using these editing traces, you need to convert them to your CRDT's local format. We do this by simulating (in memory) a set of collaborating peers. The peers fork and merge their changes. See \texttt{tools/crdt-converter} for code to perform this process using Automerge and Yjs (Yrs). We believe this algorithm could be adapted to support most existing CRDT formats and systems.
...
\begin{itemize}
    \item It is very difficult to properly compare algorithms implemented by different developers, as the implementation quality can and will often dominate performance differences that are due to the underlying collaborative algorithms being used.
    \item The set of optimisations necessary to achieve state of the art performance in collaborative editing are not widely known or published. This makes it very hard for anyone to demonstrably improve on the state of the art.
\end{itemize}
